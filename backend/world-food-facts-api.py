...
"""
Function to format result_obj data to data for frontend
"""
def format_data(result_obj):
    result = {
        "sustainabilityScore":None,
        "ecoScore":None,
        "carbonScore":None,
        "packagingScore":None,
        "certificationScore":None,
        "nutritionScore":None
    }
    result["sustainabilityScore"] = int(result_obj.score)
    result["ecoScore"] = int(result_obj.breakdown["eco"])
    result["carbonScore"] = int(result_obj.breakdown["carbon"])
    result["packagingScore"] = int(result_obj.breakdown["packaging"])
    result["certificationScore"] = int(result_obj.breakdown["certifications"])
    result["nutritionScore"] = int(result_obj.breakdown["nutrition"])
    return result
...
async def score_product():
    ...
    result = await scorer.analyze_product(product_name,category)
    if result:
        return jsonify(format_data(result))
    else:
        return jsonify({"noData":"1"})
...
"""
async def main():
    # Example Usage of SustainabilityScorer()
    scorer = SustainabilityScorer()
